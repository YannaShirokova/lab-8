...
types <- map_chr(.x = surveys, .f = typeof)
...
evals |> map_at(c("teacher_id", "weekday", "academic_degree", "seniority", "sex"), .f = as.factor) |>
  bind_cols()
...
evals |>
  ...
  mutate(
    status = case_when(
      seniority <= 4 ~ "junior",
      seniority > 4 ~ "senior")) |>
  pivot_longer(cols = c(sex, academic_degree, status),  
               names_to = "demographic", 
               values_to = "category") |>
  mutate(category = case_when(
    category == "female" ~ "Female",
    category == "male" ~ "Male",
    category == "no_dgr" ~ "No Degree",
    category == "ma" ~ "Masters",
    category == "dr" ~ "Doctorate",
    category == "prof" ~ "Professor",
    category == "junior" ~ "Junior (4 years or less)",
    category == "senior" ~ "Senior (more than 4 years)"),
    category = factor(category, 
      levels = c("Female", "Male", "Junior (4 years or less)", "Senior (more than 4 years)", 
